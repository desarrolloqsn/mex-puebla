...
    ];
  ...
  function contarTweetsYcategorias(tweets, categoriasModelos) {
    ...
    if (filtros && filtros.modelo) {
      const modelosPresentes = categoriasModelos.filter((categoriaModelo) =>
        filtros.modelo.includes(categoriaModelo.modelo)
      );
      ...
      tweets.forEach((tweet) => {
        for (const propiedad in tweet) {
          if (
            modelosPresentes.some(
              (categoriaModelo) => categoriaModelo.modelo === propiedad
            ) &&
            Array.isArray(tweet[propiedad]) &&
            tweet[propiedad].length
          )  {
    ...
    for (const propiedad in conteoPorPropiedad) {
      if (typeof conteoPorPropiedad[propiedad] === 'object') {
        const categoriasPropiedad = Object.entries(conteoPorPropiedad[propiedad]);
        categoriasPropiedad.sort((a, b) => b[1] - a[1]);
        const topCategorias = categoriasPropiedad.filter(([_, value], index) => index <= 3 && value >= 0);
        const resultado = Object.fromEntries(topCategorias);
        conteoPorPropiedad[propiedad] = resultado;
      }
      console.log('conteo por propiedad', conteoPorPropiedad)
    }
  ...
  function transformarDatosEnFormatoDeseado(resultados, categoriasModelos, tweets) {
    ...
    const modelosConValores = categoriasModelos.map((categoriaModelo) => {
      // Filtrar las categorías presentes en resultados para el modelo actual
      const categoriasPresentes = categoriaModelo.categorias
        .filter((categoria) => resultados[categoria] !== undefined);
  
      // Obtener los valores de las categorías presentes en este modelo
      const valoresCategorias = categoriasPresentes.map((categoria) => ({
        name: categoria,
        value: resultados[categoria],
      }));
  
      // Ordenar las categorías por valor y tomar las 3 con valores más altos
  const categoriasOrdenadas = valoresCategorias
  .sort((a, b) => b.value - a.value)
  .slice(0,3)
      return {
        nameModelo: categoriaModelo.modelo,
        brand: categoriaModelo.modelo,
        value: resultados[categoriaModelo.modelo] || 0,
